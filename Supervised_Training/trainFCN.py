...
experiment = Experiment("m487mKQwNTqFF4z7aZRX3Xv19", project_name="FCN_CS", log_env_gpu=True)
matplotlib.use("Agg")
...
# build model
model = FCN.build((352, 352, 3), num_of_Classes, pretrained_weights='output/weights_CS_FCN.h5')
model.summary()
...
weights_path = os.getcwd() + '/output/weights_CS_FCN.h5'
opt = Adam(lr=INIT_LR)
...
plt.title("Training Loss and Accuracy (FCN_Cityscapes)")
plt.xlabel("Epoch #")
plt.ylabel("Loss/Accuracy")
plt.legend()
plt.savefig(args["plot"] + '/FCN_plot_CS.png')

# save the model
print("[INFO] Saving the model...")
model.save_weights(args["model"] + '/' + 'FCN_CS_weights.h5')
