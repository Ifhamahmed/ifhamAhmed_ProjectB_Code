...
experiment = Experiment("m487mKQwNTqFF4z7aZRX3Xv19", project_name="AdversarialTraining_FCN_CS", log_env_gpu=True)
...
class SS_GAN(object):
    ...
    def _save_weights(self):
        ...
        """
        self.generator.save_weights('output' + '/AdvFCN_CS.h5')
    ...
    def _plot_results_and_save(self, g_loss, g_acc, d_loss, d_acc, steps, val_steps, val_loss, val_acc):
        ...
        plt.legend()
        plt.savefig('output/' + 'AdvFCN_CS_plot.png')
    ...
    def _model_checkpoint(self, prev_loss, c_loss, min_delta=0.001):
        ...
        if (prev_loss - c_loss) > min_delta:
            print('Validation Loss improved from {} to {}, saving checkpoint weights..............'.format(round(prev_loss, 5),
                                                                                                           round(c_loss,
                                                                                                               5)))
            self.generator.save_weights('output/Checkpoint_Weights_CS.h5', overwrite=True)
            new_val_loss = c_loss
        else:
...
if __name__ == '__main__':
    ...
    Semantic_GAN = SS_GAN(image_shape=(input_size[0], input_size[1]),
                          numClasses=num_of_Classes,
                          pretrained_weights='weights_CS_FCN.h5',
                          palette=palette,
                          channels=[3, 3])
